...
devServer.listen(PORT, 'localhost', (err) => {
  if (err) {
    console.error(`${colors.red('dev-server')} ${err}`)
    return
  }

  console.log(`${colors.cyan('dev-server')} Listening at http://localhost:${PORT}`)
})
...
client.stdout.on('data', (data) => {
  console.log(`${colors.cyan('electron')} ${data}`)
})

client.stderr.on('data', (data) => {
  console.error(`${colors.red('electron')} ${data}`)
})
